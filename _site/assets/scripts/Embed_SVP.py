...
def compute_costs_SVP(S_view, C_var, Q, k, l):
    ...
    for i, change in enumerate(changes):
        ...
        S_mean[:,i], S_var[:,i] = get_spatial_error_moments(S_view+candidate_block, Q)
        
    direction = changes[np.argmin(np.abs(S_var-C_var.reshape(-1,1)), axis=1)]
    wetCost = 10**13       
      
    rho = direction_to_costs(S_view, Q, direction, C_var, k, l) #not really needed since we already have means and variances stored
    rhoP1 = np.copy(rho)
    rhoM1 = np.copy(rho)
...
def Embed_SVP(C_STRUCT, payload, uni_costs=False):
    ...
    for i in range(64):
        ...
        S_view[:,k,l], betaP1[:,k,l], betaM1[:,k,l] = Embed_lattice_SVP(S_view, C_var, Q, k, l, mode_m[i], uni_rho[:,k,l])
